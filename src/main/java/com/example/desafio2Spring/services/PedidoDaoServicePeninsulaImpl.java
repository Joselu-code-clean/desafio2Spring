...
@Service("ServicioPeninsula")
public class PedidoDaoServicePeninsulaImpl implements PedidoDaoServiceI{
	...
	public void calcularPVP(Pedido pedido) {
		...
		for (int i = 0; i < pedido.getLista_producto().size(); i++) {
			pedido.getLista_producto().get(i).setPrecio_pvp(pedido.getLista_producto().get(i).getPrecio_sin_impuesto() + (pedido.getLista_producto().get(i).getPrecio_sin_impuesto() * 0.21));
		}
